...
def run_slam(src_dir, log_dir, idx, split):
    ...
    #### TODO: XXXXXXXXXXX
    t_init = 0
    while abs(slam.lidar[t_init]['t'] < slam.joint['t'][0]):
        t_init += 1
    ...
    xyth_vals = []
    for each in d:
        xyth_vals.append([*each["xyth"]])
    xyth_vals = np.array(xyth_vals)
    ...
    grid_p_x = []
    grid_p_y = []
    mp_plt = plt.subplot(111)
    n_x = []
    n_y = []

    for k in tqdm.tqdm(range(t_init, T)):
        slam.dynamics_step(k)
        slam.observation_step(k)
        grid_p_x.append(slam.grid_max[0][0])
        grid_p_y.append(slam.grid_max[1][0])
        if k % 500 == 0 and k > 5:
            arr_map = slam.map.grid_cell_from_xy(grid_p_x, grid_p_y, True)
            odo_map = slam.map.grid_cell_from_xy(odo_x, odo_y, True)
            plt.imshow(slam.map.log_odds)

            mp_plt.plot(odo_map[:, 1], odo_map[:, 0], '.r', markersize=3)
            mp_plt.plot(arr_map[:, 1], arr_map[:, 0], '.b', markersize=3)

            # plt.xlim([0, 800])
            # plt.ylim([800, 0])
            # plt.hold(True)

            plt.savefig(os.path.join(log_dir, 'map3.png'))
...
@click.option('--idx', default='2', help='dataset number', type=int)
