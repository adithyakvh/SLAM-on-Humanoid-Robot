...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y, out_call=False):
        """
        ...
        y = np.clip(y, s.xmin, s.xmax)
        x_grid = np.ceil((x - s.xmin)/s.resolution)
        y_grid = np.ceil((y-s.ymin)/s.resolution)
        if out_call == True:
            grid_cell = np.vstack((x_grid.astype(int), y_grid.astype(int))).T
            return np.array(grid_cell)

        grid_cell = np.vstack((x_grid.astype(int), y_grid.astype(int)))
        return grid_cell
...
class slam_t:
    ...
    def stratified_resampling(p, w):
        ...
        weight = w[0]
        iter = 0
        weight_update = copy.deepcopy(w)
        pose_iter = copy.deepcopy(p)
        count = 0
        while count<p.shape[1]:
            count += 1
            cent = ((each - 1) * 1/ n) + rnd
            ...
            pose_iter[:, count] = pose_iter[:, iter]
            weight_update[count] = weight_update[iter]

        return pose_iter, weight_update
    ...
    def rays2world(s, p, d, head_angle=0, neck_angle=0, angles=None):
        ...
        # the data
        s.out_call = False
        correct_index = np.where((s.lidar_dmin<=d) & (d<=s.lidar_dmax))
        angles = angles[correct_index]
        d = d[correct_index]

        # 1. from lidar distances to points in the LiDAR frame
        end_points = np.vstack((d * np.cos(angles), d * np.sin(angles), np.ones((d.shape[0]))))
        end_points_homogenous = make_homogeneous_coords_3d(end_points)

        # 2. from LiDAR frame to the body frame
        rotation_Lidar_Body = euler_to_se3(0, head_angle, neck_angle,  np.array([0, 0, s.lidar_height]))
        end_points_body = rotation_Lidar_Body @ end_points_homogenous

        # 3. from body frame to world frame
        rotation_body_world = euler_to_se3(0, 0, p[2], np.array([p[0], p[1], 1.263]))
        end_points_body_homogenous = make_homogeneous_coords_3d(end_points_body[:3, :])
        end_points_world = rotation_body_world @ end_points_body_homogenous
        return end_points_world[:2, :]
    ...
    def get_control(s, t):
        ...
        control = smart_minus_2d(pose_t, pose_t_1)
        return control
        #### TODO: XXXXXXXXXXX
        # raise NotImplementedError
    ...
    def dynamics_step(s, t):
        ...
        """
        #### TODO: XXXXXXXXXXX
        pose = s.get_control(t)
        noise = np.random.multivariate_normal([0, 0, 0], s.Q)
        for i in range(s.n):
            s.p[:, i] = smart_plus_2d(smart_plus_2d(s.p[:, i], pose), noise)
    ...
    def update_weights(w, obs_logp):
        ...
        """
        #### TODO: XXXXXXXXXXX
        log_w_upd = np.log(w) + obs_logp
        q = slam_t.log_sum_exp(log_w_upd)
        w_update = log_w_upd - q
    ...
    def observation_step(s, t):
        ...
        """
        #### TODO: XXXXXXXXXXX
        head_angle = s.joint['head_angles'][1][t]
        neck_angle = s.joint['head_angles'][0][t]
        d = s.lidar[t]['scan']
        grids = []
        logprob_obs = np.zeros(s.n)

        if t == 0:
            d_projected_world = s.rays2world(s.p[:, 0], d, head_angle, neck_angle, s.lidar_angles)
            d_occupied_grid = s.map.grid_cell_from_xy(d_projected_world[0, :], d_projected_world[1, :])
            # s.map.cells[d_occupied_grid[0:2, :, :]] = np.array([1, 1])
            s.map.cells[d_occupied_grid[0, :], d_occupied_grid[1, :]] = 1

        # else:
        for i in range(s.n):
            d_projected_world = s.rays2world(s.p[:, i], d, head_angle, neck_angle, s.lidar_angles)
            d_occupied_grid = s.map.grid_cell_from_xy(d_projected_world[0, :], d_projected_world[1, :])
            logprob_obs[i] = np.sum(s.map.cells[d_occupied_grid[0, :], d_occupied_grid[1, :]])
            grids.append(d_occupied_grid)

        # if t == 0:
        #     for each_cell in grids:
        #         cell_x = each_cell[0]
        #         cell_y = each_cell[1]
        #         s.map.cells[cell_x][cell_y] = 1
        #     cell_sum[i] = np.sum(s.map.cells)
        # else:
        #     cell_sum[i] = np.sum(s.map.cells[d_occupied_grid[0, :], d_occupied_grid[1, :]])


        s.w = s.update_weights(s.w, logprob_obs)
        largest_weight = np.argmax(s.w)
        largest_particle = s.p[:, largest_weight]
        # print("largest_particle", largest_particle)
        # largest_world = s.rays2world(largest_particle, d, head_angle, neck_angle, s.lidar_angles)
        # largest_particle_grid = s.map.grid_cell_from_xy(largest_world[0,:], largest_world[1,:])

        # s.map.log_odds += s.lidar_log_odds_free
        # s.map.log_odds[]
        s.grid_max = s.map.grid_cell_from_xy(largest_particle[0], largest_particle[1])
        grid_horizontal, grid_vertical = grids[largest_weight]

        flattened_x = np.linspace([s.grid_max[0][0]] * len(grid_vertical),
                                  grid_vertical - 1, endpoint=False).flatten().astype(int)

        flattened_y = np.linspace([s.grid_max[1][0]] * len(grid_vertical),
                                  grid_vertical - 1, endpoint=False).flatten().astype(int)


        s.map.log_odds[flattened_x, flattened_y] = s.map.log_odds[flattened_x, flattened_y] + \
                                                    (0.3 * s.lidar_log_odds_free)
        s.map.log_odds[grid_horizontal, grid_vertical] = s.map.log_odds[grid_horizontal, grid_vertical] + \
                                                         s.lidar_log_odds_occ

        min_value = -s.map.log_odds_max
        max_value = s.map.log_odds_max
        np.clip(s.map.log_odds, min_value, max_value)
        greater_threshold = np.where(s.map.log_odds >= s.map.log_odds_thresh)
        lesser_threshold = np.where(s.map.log_odds < s.map.log_odds_thresh)
        s.map.cells[greater_threshold] = 1
        s.map.cells[lesser_threshold] = 0
        # grid_locations = s.map.grid_cell_from_xy(s.p[:, np.argmax(s.w)][0], s.p[:, np.argmax(s.w)][1])
